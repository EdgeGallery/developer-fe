...
<template>
  <div class="capManagement padding-inner">
    <div class="system-title">
      {{ $t('devSystem.capability.systemMep') }}
      <el-button
        id="btn_addCapability"
        class="common-add-btn rt"
        @click="handleShowForm"
      >
        {{ $t('devSystem.capability.addMep') }}
      </el-button>
    </div>
    <el-dialog
    ...
    >
      ...
      >
        <h3 class="common-dlg-title">
          {{ $t('devSystem.capability.basicInfo') }}
        </h3>
        <el-form-item
          :label="$t('devSystem.capability.oneLevelCapability')"
          prop="group.name"
          class="w50 lt"
        >
          <el-select
        ...
        <el-form-item
          :label="$t('devSystem.capability.twoLevelCapability')"
          prop="name"
          class="w50 lt"
        >
          <el-input
            :placeholder="$t('devSystem.capability.zh_cn')"
            v-model="form.name"
          />
        </el-form-item>
        <el-form-item
        ...
        >
          <el-input
            :placeholder="$t('devSystem.capability.en')"
            v-model="form.nameEn"
          />
        </el-form-item>
        <el-form-item
        ...
        <el-form-item
          :label="$t('devSystem.capability.apiFileId')"
          prop="apiFileId"
          ref="apiFileItem"
        >
          <el-upload
          ...
          >
            <el-button
              ...
              class="featuresBtn"
            >
              {{ $t('devSystem.upload') + $t('devSystem.capability.apiFileId') }}
            </el-button>
            <el-tooltip
              effect="dark"
              :content="this.$t('devSystem.capability.apiText')"
              placement="right"
            >
        ...
        <div class="clear lt">
          <el-form-item
            :label="$t('devSystem.capability.guideFileId')"
            prop="guideFileId"
            ref="guideFileItem"
            class="w50 lt"
          >
            ...
            >
              <el-button
                ...
                class="featuresBtn"
              >
                {{ $t('devSystem.capability.guideFileId_zh') }}
              </el-button>
              <el-tooltip
                effect="dark"
                :content="this.$t('devSystem.capability.apiFunctionMd')"
                placement="right"
              >
                <span class="common-info tip-info" />
              </el-tooltip>
          ...
          >
            ...
            >
              <el-button
                ...
                class="featuresBtn"
              >
                {{ $t('devSystem.capability.guideFileId_en') }}
              </el-button>
              <el-tooltip
                effect="dark"
                :content="this.$t('devSystem.capability.apiFunctionMd')"
          ...
          <el-form-item
            :label="$t('devSystem.capability.icon')"
            class="icon"
            ref="iconFileItem"
            prop="appIcon"
          >
            <div class="default-icon">
              ...
              >
                ...
                />
                <span>{{ $t('devSystem.capability.defaultIcon') }}</span>
              </div>
            </div>
            <el-upload
              id="projectLogo"
            ...
            <span class="uploadIconSpan">{{ $t('devSystem.capability.customIcon') }}</span>
            <el-tooltip
              class="item"
              effect="dark"
              :content="this.$t('devSystem.capability.limitition')"
              placement="right"
            >
              <span class="common-info tip-info" />
            </el-tooltip>
            <div
              ...
              v-if="showErr"
            >
              {{ $t('devSystem.capability.iconRequired') }}
            </div>
          </el-form-item>
        </div>
        <h3 class="common-dlg-title">
          {{ $t('devSystem.capability.registerInfo') }}
        </h3>
        <el-form-item
          :label="$t('devSystem.capability.serviceName')"
          prop="host"
          class="w50"
        >
          <el-input
        ...
        <el-form-item
          :label="$t('devSystem.capability.inPort')"
          prop="port"
          class="w50"
        >
          <el-input
        ...
        <el-form-item
          :label="$t('devSystem.capability.protocol')"
          prop="protocol"
          class="w50"
        >
          <el-select
        ...
        <el-form-item
          :label="$t('devSystem.capability.provider')"
          prop="provider"
          class="w50"
        >
          <el-input
        ...
        <el-form-item
          :label="$t('devSystem.capability.onlineExperience')+' url'"
          class="w50"
        >
          <el-input
            size="small"
    ...
    <div class="list common-div-bg clear">
      <div class="title">
        <span>
          <el-input
            size="medium"
            v-model="enterQuery"
            :placeholder="$t('devSystem.capability.serviceName')"
            class="search_input"
      ...
      >
        ...
        <el-table-column
          prop="group.name"
          :label="$t('devSystem.capability.capType')"
          min-width="14%"
        >
          <template slot-scope="scope">
...
export default {
  ...
  data () {
    ...
    const validateName = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        callback(new Error(this.$t('devSystem.promptMessage.systemCapaNameCn')))
      } else {
        callback()
      }
    }
    const validateNameEn = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        callback(new Error(this.$t('devSystem.promptMessage.systemCapaNameEn')))
      } else {
        callback()
      }
    }
    const validatePort = (rule, value, callback) => {
      ...
      if (!value) {
        callback(new Error(`${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.capability.inPort')}`))
      } else if (!reg.test(value)) {
        callback(new Error(`${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.capability.correct')}${this.$t('devSystem.capability.inPort')}`))
      } else {
        callback()
      }
    }
    const validateDescription = (rule, value, callback) => {
      let reg = /^(?!\s)[\S.\s\n\r]{1,200}$/g
      if (!value) {
        callback(new Error(`${this.$t('devSystem.pleaseInput')}${this.$t('common.describe')}`))
      } else if (!reg.test(value)) {
        callback(new Error(this.$t('devSystem.promptMessage.systemCapaDescCn')))
      } else {
        callback()
      }
    }
    const validateDescriptionEn = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        callback(new Error(this.$t('devSystem.promptMessage.systemCapaDescEn')))
      } else {
        callback()
      }
    }
    const validateAppIcon = (rule, value, callback) => {
      if (!value) {
        callback(new Error(this.$t('devSystem.capability.iconRequired')))
      } else {
        callback()
      }
    }
    const validateHost = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        callback(new Error(this.$t('devSystem.promptMessage.systemServiceName')))
      } else {
        callback()
      }
    }
    const validateVersion = (rule, value, callback) => {
      ...
      if (!value) {
        ...
        callback(new Error(`${this.$t('devSystem.pleaseInput')}1~20 ${this.$t('devSystem.capability.char')}`))
      } else {
        callback()
      }
    }
    const validateProvider = (rule, value, callback) => {
      let reg = /^[\S\s]{1,20}$/g
      if (!value) {
        callback(new Error(`${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.capability.provider')}`))
      } else if (!reg.test(value)) {
        callback(new Error(this.$t('devSystem.promptMessage.systemProviderName')))
      } else {
        callback()
      }
    }
    return {
      ...
      rules: {
        apiFileId: [
          { required: true, validator: (r, v, callback) => { validate(['apiFileId'], callback, this.$t('devSystem.promptMessage.uploadApiFile')) }, trigger: 'change' }
        ],
        guideFileId: [
          { required: true, validator: (r, v, callback) => { validate(['guideFileId'], callback, this.$t('devSystem.promptMessage.systemDocument')) }, trigger: 'change' }
        ],
        guideFileIdEn: [
          { required: true, validator: (r, v, callback) => { validate(['guideFileIdEn'], callback, this.$t('devSystem.promptMessage.systemDocumentEn')) }, trigger: 'change' }
        ],
        name: [
        ...
        protocol: [
          { required: true, message: `${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.capability.protocol')}` }
        ],
        host: [
  ...
  methods: {
    ...
    getIconFileId () {
      ...
      Capability.postIconFileIdApi('icon', formdata).then(res => {
    ...
    handleChangeLogo (file) {
      ...
      if (file) {
        if (file.raw.name.indexOf(' ') !== -1) {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.fileNameType'), 'warning')
          this.logoFileList = []
        } else {
        ...
        if (file.size / 1024 / 1024 > 2) {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.moreThan2'), 'warning')
          this.logoFileList = []
        }
        ...
        if (fileTypeArr.indexOf(this.fileType.toLowerCase()) === -1) {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.checkFileType'), 'warning')
          this.logoFileList = []
        }
      }
    ...
    handleExceed (file, fileList) {
      if (fileList.length === 1) {
        this.$eg_messagebox(this.$t('devSystem.promptMessage.onlyOneFile'), 'warning')
      }
    },
    getBase64Image (img) {
    ...
    setDivHeight () {
      common.setDivHeightFun(this.screenHeight, 'list', 255)
    },
    handleDelete (row) {
    ...
    getIcon (fileId) {
      return Capability.getCapabilityIconApi(fileId)
    },
    getFileList (fileType, fileId) {
      Capability.getApiFileApi(fileId).then(res => {
        let obj = { name: '' }
    ...
    createCapabilityFun (data) {
      Capability.createCapability(data).then(res => {
        if (res && res.data && res.data.id) {
          this.$eg_messagebox(this.$t('devSystem.capability.addMep') + this.$t('devSystem.success'), 'success')
          this.onClose()
        ...
        if (err.response.data.message === 'The capability is exist') {
          this.$eg_messagebox(this.$t('devSystem.capability.isExistCapability'), 'error')
        } else {
          this.$eg_messagebox(this.$t('devSystem.capability.addMep') + this.$t('devSystem.fail'), 'error')
        }
      })
    },
    editCapabilityFun (data) {
      Capability.editCapability(this.form.id, data).then(() => {
        this.$eg_messagebox(this.$t('common.edit') + this.$t('devSystem.success'), 'success')
        this.onClose()
        this.$refs['form'].resetFields()
        this.getListData()
      }).catch(() => {
        this.$eg_messagebox(this.$t('common.edit') + this.$t('devSystem.fail'), 'error')
      })
    },
    onSubmit () {
    ...
    submitFile (key, fileList, fileType) {
      ...
      this.loading = true
      Capability.postIconFileIdApi(fileType, fd).then(res => {
        if (res.data.fileId) {
        ...
        if (error && error.response && error.response.data.code === 403) {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.guestPrompt'), 'error')
        } else {
